...
\begin{lstlisting}[language=phpstyle, label=lst:checkTemplate, caption={\lstinline|checkTemplate| unit test}]
...
\begin{lstlisting}[language=phpstyle, label=lst:testCheckFeeds, caption={\lstinline|checkFeeds| unit tests}]
...
\end{lstlisting}%$
\begin{description}
\item[Line 1] PHPUnit is informed that this test is dependent of \lstinline|testCreateUser()|.
\item[Line 2] Since this test is dependent of \lstinline|testCreateUser()| it takes the \lstinline|userId| that\linebreak \lstinline|testCreateUser()| returns as a parameter.
...
\item[Line 29] A new feed is made for testing using the current time as a time stamp.

...
\item IL means "identification length", if the flag is set then the $ID\_LENGTH$ field is present in the header as a single octet.\todo{Hmm?} 
\item The TNF value indicates the structure of the TYPE field. The format follows the RTD type name which has been defined in the NFC Forum RTD specification\citep{ndef}\todo{Hmm?}
\end{itemize}
...
The encoding mode consists of four bits, indicating what encoding mode is used and to convey other information. Some of the indicators are:
